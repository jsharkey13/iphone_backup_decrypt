...
from iphone_backup_decrypt.utils import _safe_output_path
...
class SafeOutputPathTests(unittest.TestCase):
    ...
    def test_allows_path_below_output_folder(self):
        result = _safe_output_path(self.output_folder, "HomeDomain", "Library", "file.db")
        self.assertEqual(
            result,
    ...
    def test_rejects_parent_traversal(self):
        with self.assertRaises(ValueError):
            _safe_output_path(self.output_folder, os.pardir, "outside", "file.db")

    def test_rejects_absolute_path(self):
        unsafe_path = os.path.join(os.path.abspath(os.sep), "outside", "file.db")
        with self.assertRaises(ValueError):
            _safe_output_path(self.output_folder, unsafe_path)
